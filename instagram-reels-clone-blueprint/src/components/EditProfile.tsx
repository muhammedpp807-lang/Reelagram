...
export function EditProfile() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      const error = await updateProfile({ username, fullName, bio, avatar });
      ...
      <form onSubmit={submit} className="space-y-6">
        ...
        <div className="flex items-center gap-2 justify-end">
          ...
          <button
            type="submit"
            disabled={busy}
            className="text-sm font-semibold px-4 py-2 rounded-md bg-blue-500 hover:bg-blue-600 disabled:opacity-50"
          >
