  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "login") {
        ...
        const err = await signup({ username, email, password, fullName });
        if (err) setError(err);
      }
    } finally {
      setBusy(false);
        ...
        <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-6">
          <form onSubmit={submit} className="space-y-2.5">
            ...
            <button
              type="submit"
              disabled={busy}
              className="w-full mt-3 py-2 rounded-lg bg-blue-500 hover:bg-blue-600 font-semibold text-sm transition disabled:opacity-50"
            >
